...
//        public ApiWebClient(string baseUrl= "https://anypoint.mulesoft.com/mocking/api/v1/links/3b9589d5-0bed-46a1-9354-d230355d957f/exp", bool usingOAuth = false, string OAuthClientSecret = null, string pingFedUrl = "https://sso.ameren.com")
//        {
//            try
//            {
//                _baseUrl = baseUrl;
...
//        public async Task<ApiResponse<IEnumerable<T>>> GetList<T>(APIRequest request)
//        {
//            var apiResponse = new ApiResponse<IEnumerable<T>>();
//            string url = FormattedUrl(request.endpoint, request.QueryParameters);
//            string jsonString = "";

//            var response = ExecuteHttpClientGet(request.endpoint, request.HeaderParameters);
...
//                    return await GetList<T>(request);
//                }
//                else
//                {
//                    apiResponse.ErrorCode = $"{(int)response.StatusCode}: {response.StatusCode.ToString()} - {response.ReasonPhrase}";
...
//                {
//                    jsonString = await response.Content.ReadAsStringAsync();
//                    apiResponse.Data = JsonConvert.DeserializeObject<IEnumerable<T>>(jsonString);
//                }
//                catch (Exception ex)
//                {
//                    apiResponse.ErrorMessage = $"API GET Error: Description Response: {jsonString}";
